...
import enable_pretty_logging# type: ignore
from tornado.web
...
if False:
  from typing
import Any, Dict, List, Set, Tuple# noqa: F401

global verify_ssl_cert

def gen_server_name(server, port):
  if len(server.split('/')) > 3: #true
...
path_to_mode = {
  ...
  '/perWord': 'perWord',
  '/coverage': 'coverage',
  '/listLanguageNames': 'languageNames',
  '/identifyLang': 'identifyLang',
  '/getLocale': 'getLocale',
}
...
if self.request.path != '/listPairs'
and self.request.path != '/list':
  self.send_error(400)
else :
...
elif query == 'analyzers'
or query == 'analysers':
  self.send_response({
    pair: self.server_lang_pair_map['analyzers'][pair][0]
    for pair in self.server_lang_pair_map['analyzers']
  })
elif query == 'generators':
  self.send_response({
    pair: self.server_lang_pair_map['generators'][pair][0]
    for pair in self.server_lang_pair_map['generators']
  })
elif query == 'taggers'
or query == 'disambiguators':
  self.send_response({
    pair: self.server_lang_pair_map['taggers'][pair][0]
    for pair in self.server_lang_pair_map['taggers']
  })
else :
  self.send_error(400)
...
def get_server(self, lang_pair, mode = 'pairs', * args, ** kwargs): #when we get a / perWord request, we have multiple modes, all of which have to be on the server# the modes will not be 'pairs'
...
class WeightedRandom(Balancer):
  def __init__(self, servers):
  self.serverlist = OrderedDict([(server, 0) for server in servers])
all_test_results = [test_server_pool([server[0]
...
test_score = sum([result[1]
  for testPath, result in results.items()
])
...
all_test_results = [test_server_pool([server[0]
  for server in self.serverlist.items()
]) for _ in range(0, self.num_responses)]
for test_results in all_test_results:
  for test_result in test_results.items():
  server = test_result[0]
results = test_result[1]
test_sum = sum([result[1]
  for testPath, result in results.items()
])
if '/list'
not in self.serverlist[server][1]:
...
def test_server_pool(server_list):
  tests = {
    '/list?q=pairs': lambda x: isinstance(x, dict) and
    set(x.keys()) == {
      'responseStatus',
      'responseData',
      'responseDetails'
    ...
    all([set(lang_pair.keys()) == {
        'sourceLanguage',
        'targetLanguage'
      }
      for lang_pair in x['responseData']
    ]),
...
type: Dict[Any, Dict[str, Tuple[bool, float]]]
...
function is a little complex, better illustrated than described:
  capabilities = {
    'pairs': {#
      note that pairs is a special mode compared to taggers / generators / analyzers('lang', 'pair'): [(server1, port1), (server2, port2)]
    }
...
on scaleMT survers.#You will probably need to batch - translate or batch - analyze with the language pairs from / listPairs and# look at the
return codes in order to do this.
  http = tornado.httpclient.HTTPClient()
...
logging.info('Getting information from %s', request_url)# make the request
try:
result = http.fetch(request_url, request_timeout = 15, validate_cert = verify_ssl_cert)
except Exception as e:
...
if 'responseStatus'
not in response or response['responseStatus'] != 200 or 'responseData'
not in response:
...
parser.add_argument('-t', '--tests', help = 'perform tests on server pool', action = 'store_true',
  default = False)
parser.add_argument('-p', '--port', help = 'port to run gateway on (default = 2738)', type = int,
  default = 2738)
parser.add_argument('-c', '--ssl-cert', help = 'path to SSL Certificate',
  default = None)
parser.add_argument('-k', '--ssl-key', help = 'path to SSL Key File',
  default = None)
parser.add_argument('-d', '--debug', help = 'debug mode (do not verify SSL certs)', action = 'store_false',
  default = True)
parser.add_argument('-j', '--num-processes', help = 'number of processes to run (default = number of cores)', type = int,
  default = 0)
parser.add_argument('-i', '--test-interval', help = 'interval to perform tests in ms (default = 3600000)', type = int,
  default = 3600000)
...
logging.info('Using server language-pair mapping: %s', server_lang_pair_map)# balancer = RoundRobin(server_port_list, server_lang_pair_map)# balancer = LeastConnections(server_port_list)# balancer = WeightedRandom(server_port_list)
balancer = Fastest(server_port_list, server_lang_pair_map, 5)

application = tornado.web.Application([
  (r '/list', ListRequestHandler, {
    'serverLangPairMap': server_lang_pair_map
  }),
  (r '/listPairs', ListRequestHandler, {
    'serverLangPairMap': server_lang_pair_map
  }),
  (r '/.*', RedirectRequestHandler, {
    'balancer': balancer
  }),
])
