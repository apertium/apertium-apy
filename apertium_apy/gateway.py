...
# when we get a / perWord request, we have multiple modes, all of which have to be on the server# the modes will not be 'pairs'
...
def test_server_pool(server_list):
    tests = {
      ...
      and all([isinstance(lang_pair, dict) for lang_pair in x['responseData']])
        and all([set(lang_pair.keys()) == {
            'sourceLanguage',
            'targetLanguage'
        }
...
# balancer = RoundRobin(server_port_list, server_lang_pair_map)# balancer
# = LeastConnections(server_port_list)# balancer =
# WeightedRandom(server_port_list)
